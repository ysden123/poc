...
public class HttpServer extends AbstractVerticle {
	...
	public void start(Future<Void> startFuture) {
		...
		getIndexCounter = Counter
				.build("wp_server2_get", "WebPals counter for get method in Server2")
				.register(registry);
		getIndexSuccededCounter = Counter
				.build("wp_server2_get_succeded", "WebPals counter for succeded get method in Server2")
				.register(registry);
		getIndexFailedCounter = Counter
				.build("wp_server2_get_failed", "WebPals counter for failed get method in Server2")
				.register(registry);
	...
	private void serviceHandler(RoutingContext routingContext) {
		...
		vertx.eventBus().send(Service.EB_SERVICE_ADDRESS, "Get page",
