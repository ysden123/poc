...
object PlainQueries {
  ...
  def printAllCoffees(db: Database): Unit = {
    ...
    Await.result(
      db.run(sql"""select * from coffees""".as[(String, Double, Int, Int,Long)]).map(_.foreach(r => println(s"${r._1}, ${r._2}, ${r._3}, ${r._4}, ${r._5}"))),
      Duration.Inf)
    println("<==printAllCoffees")
  }
  ...
  def printAllCoffees2(db: Database): Unit = {
    ...
    Await.result(
      db.run(sql"""select * from coffees""".as[(String, Double, Int, Int,Long)]).map(_.map(r => Coffee(r._1, r._2, r._3, r._4, r._5))
        .foreach(println)),
      Duration.Inf
    )
    println("<==printAllCoffees2")
